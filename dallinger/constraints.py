...
@click.option(
    ...
    "extras",
    multiple=True,
    help="Include optional dependencies from pyproject.toml (may be repeated).",
)
def check(extras):
    """
...
def check_constraints(extras=None):
    """
    Check the working directory to see whether a constraints.txt file exists and is up to date. Raises a ValueError if not.
    """
    ...
    input_path = _find_input_path(prefer_pyproject=bool(extras))
    _validate_extras(input_path, extras)

    if not _constraints_up_to_date(input_path, extras):
        raise ValueError(
...
def generate_constraints(extras=None):
    """
    ...
    If the input file is a pyproject.toml, optional dependency groups can be included
    by passing ``extras``. Extras are ignored for requirements.txt inputs.
...
def ensure_constraints_file_presence(directory, extras=None):
    """
    ...
    if constraints_path.exists():
        if not _constraints_autogenerated():
            ...
            return
        if _constraints_up_to_date(input_path, extras):
...
def _find_input_path(prefer_pyproject: bool = False) -> Path:
    if prefer_pyproject and pyproject_path.exists():
...
def _get_md5sum(input_path: Path, extras=None) -> str:
    with open(input_path, "rb") as f:
        file_bytes = f.read()
    extras = _normalize_extras(extras)
    if extras:
        extras_bytes = f"\nextras:{','.join(extras)}".encode("utf-8")
        file_bytes += extras_bytes
...
def _constraints_up_to_date(input_path: Path, extras=None) -> bool:
    assert_python_version_file_presence()
    python_version = _get_requested_python_version()
    return (
        f"Python {python_version}\n" in constraints_path.read_text()
        and _get_md5sum(input_path, extras=extras) in constraints_path.read_text()
    )
...
def _get_dallinger_reference(input_path: Path, extras=None) -> str:
...
def _get_implied_dallinger_reference(input_path: Path, extras=None) -> str:
...
def _pip_compile(
    in_file, out_file, constraints: Optional[list] = None, extras=None
):
    ...
    extras = _normalize_extras(extras)
    md5sum = _get_md5sum(in_file, extras=extras)
    extras_info = f" and extras {', '.join(extras)}" if extras else ""
    compile_info = (
        "dallinger constraints generate\n#\n"
        f"# Compiled from a {Path(in_file).name} file with md5sum {md5sum}"
        f"{extras_info} and a .python-version file requesting Python {requested_python_version}"
    )
...
def _normalize_extras(extras) -> list[str]:
    if not extras:
        return []
    normalized = []
    for extra in extras:
...
def _validate_extras(input_path: Path, extras) -> None:
    if not extras:
        return
    if input_path != pyproject_path:
        raise ValueError(
            "Extras can only be used when pyproject.toml is the input file. "
            "Remove requirements.txt or use --extra with a pyproject-based project."
        )
